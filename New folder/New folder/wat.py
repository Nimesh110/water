...
with col1:
    ...
    status=st.radio("Select an option:",['Readings'],horizontal=True)

    #input logic
    if status == "Readings":
        initReading =st.number_input("Enter last month reading",key="pre",min_value=0,step=1,format="%d")
        finReading=st.number_input("Enter this month reading",key="cur",min_value=0,step=1,format="%d")
